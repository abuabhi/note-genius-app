...
export class OneNoteDocumentAuthService extends DocumentAuthService {
  private static instance: OneNoteDocumentAuthService | null = null;
  ...
  static getInstance(): OneNoteDocumentAuthService {
    if (!OneNoteDocumentAuthService.instance) {
      OneNoteDocumentAuthService.instance = new OneNoteDocumentAuthService();
    }
    return OneNoteDocumentAuthService.instance;
  }
  ...
  async authenticate(): Promise<AuthCredentials> {
    ...
    try {
      ...
      const configResponse = await fetch(`https://zuhcmwujzfddmafozubd.supabase.co/functions/v1/microsoft-auth`, {
        method: 'POST',
        headers: {
      ...
      if (!configData?.client_id) {
        throw new Error('Unable to get Microsoft OAuth configuration');
      }

      // Create auth URL
      const REDIRECT_URI = `${window.location.origin}/auth/microsoft/callback`;
      const SCOPE = 'https://graph.microsoft.com/User.Read https://graph.microsoft.com/Notes.Read https://graph.microsoft.com/Notes.ReadWrite';
      const state = Math.random().toString(36).substring(2);
      
      sessionStorage.setItem(`${this.storagePrefix}auth_state`, state);

      const authUrl = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${configData.client_id}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_mode=query&scope=${encodeURIComponent(SCOPE)}&state=${state}&prompt=select_account`;
      ...
      console.log('🚀 [ONENOTE] Authentication popup opened');
  ...
  private async waitForAuthResult(popup: Window): Promise<AuthCredentials> {
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = async (event: MessageEvent) => {
        if (event.origin !== window.location.origin || event.data?.type !== 'microsoft_oauth_callback') {
          return;
        }
        ...
        console.log('📨 [ONENOTE] OAuth callback received:', event.data);

        // Close the popup immediately after receiving the message
        if (!popup.closed) {
          console.log('🔚 [ONENOTE] Closing popup after receiving callback');
          popup.close();
        }
      ...
      const pollForResult = () => {
        const pendingResult = localStorage.getItem('onenote_auth_pending') || localStorage.getItem(`${this.storagePrefix}auth_pending`);
        if (pendingResult && !resolved) {
          try {
            const authData = JSON.parse(pendingResult);
            localStorage.removeItem('onenote_auth_pending');
            ...
            if (!popup.closed) {
              console.log('🔚 [ONENOTE] Closing popup after polling result');
              popup.close();
            }
  ...
  private async exchangeCodeForTokens(code: string): Promise<AuthCredentials> {
    ...
    const response = await fetch(`https://zuhcmwujzfddmafozubd.supabase.co/functions/v1/microsoft-auth`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp1aGNtd3VqemZkZG1hZm96dWJkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDY1MjUxOTQsImV4cCI6MjA2MjEwMTE5NH0.oz_MnWdGGh76eOjQ2k69OhQhqBh4KXG0Wq_cN-VJwzw',
    ...
    return {
      ...
      userName: userData.displayName || userData.mail || userData.userPrincipalName,
  ...
  async disconnect(): Promise<void> {
    console.log('🔌 [ONENOTE] Disconnecting...');
