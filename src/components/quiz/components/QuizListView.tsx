...
export const QuizListView: React.FC<QuizListViewProps> = ({
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        {[...Array(5)].map((_, i) => (
          <Card key={i} className="animate-pulse border-mint-100">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex-1 space-y-2">
                  <div className="h-6 bg-mint-100 rounded w-1/2"></div>
                ...
                <div className="flex gap-2">
                  <div className="h-8 bg-mint-100 rounded w-24"></div>
  ...
  const handleFavoriteClick = (e: React.MouseEvent, quizId: string) => {
    e.preventDefault();
    e.stopPropagation();
