...
export const useAuthRedirects = ({
  ...
  useEffect(() => {
    // Add a small delay to prevent race conditions during signup flow
    const redirectTimer = setTimeout(() => {
      ...
      // Check for signup completion flag for tier-selection access with detailed logging
      const signupInfo = sessionStorage.getItem('signup_completed');
      let hasRecentSignup = false;
      
      console.log('🚦 [AUTH REDIRECTS] Checking signup info:', signupInfo);
      
      if (signupInfo) {
        try {
          const parsed = JSON.parse(signupInfo);
          hasRecentSignup = Date.now() - parsed.timestamp < 10 * 60 * 1000; // Valid for 10 minutes
          console.log('🚦 [AUTH REDIRECTS] Recent signup detected:', { 
            hasRecentSignup, 
            email: parsed.email,
            timeSinceSignup: Math.round((Date.now() - parsed.timestamp) / 1000) + 's ago'
          });
        } catch (error) {
          console.log('🚦 [AUTH REDIRECTS] Invalid signup info, clearing');
    ...
    }, 100); // Small delay to allow signup flow to complete
