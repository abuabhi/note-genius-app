...
export const combineFlashcardOperations = (
  ...
  // Memoize the recordFlashcardReview function to prevent recreation
  const recordFlashcardReview = useCallback(async (flashcardId: string, score: 0 | 1 | 2 | 3 | 4 | 5) => {
    if (!state.user) {
  ...
  // Memoize the combined operations to prevent recreation
  return useMemo(() => ({
    ...flashcardOperations,
