-- Fix database functions broken by security changes (search_path = '')
-- Add public schema prefixes to all table references
...
CREATE OR REPLACE FUNCTION public.filter_user_quizzes(p_user_id uuid, p_search_term text DEFAULT ''::text, p_subject_name text DEFAULT 'all'::text, p_sort_by text DEFAULT 'newest'::text, p_page_num integer DEFAULT 0, p_page_size integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  WITH filtered_quizzes AS (
    ...
    LEFT JOIN public.user_subjects us ON us.id = q.subject_id
