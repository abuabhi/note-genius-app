...
-- Create table to track Supadata.ai API usage for credit monitoring
CREATE TABLE public.supadata_usage (
...
-- Add RLS policies
ALTER TABLE public.supadata_usage ENABLE ROW LEVEL SECURITY;

-- Policy for users to view their own usage (for future user dashboards)
CREATE POLICY "Users can view their own usage" 
  ON public.supadata_usage 
  FOR SELECT 
...
CREATE INDEX idx_supadata_usage_month_year ON public.supadata_usage(month_year);
CREATE INDEX idx_supadata_usage_user_month ON public.supadata_usage(user_id, month_year);

-- Function to get total credits used this month
CREATE OR REPLACE FUNCTION public.get_supadata_credits_used_this_month()
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION public.get_supadata_usage_stats(target_month TEXT DEFAULT to_char(now(), 'YYYY-MM'))
RETURNS TABLE(
