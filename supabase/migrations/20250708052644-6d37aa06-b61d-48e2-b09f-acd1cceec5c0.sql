...
-- Note: Using regular CREATE INDEX since CONCURRENTLY cannot run in transaction
...
CREATE INDEX IF NOT EXISTS idx_notes_user_pinned_updated ON notes(user_id, pinned, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_notes_user_source_type ON notes(user_id, source_type);

-- Add user_subjects index for faster subject lookups
CREATE INDEX IF NOT EXISTS idx_user_subjects_user_name ON user_subjects(user_id, name);

-- Add indexes for note enhancement features
CREATE INDEX IF NOT EXISTS idx_note_enrichment_usage_note_user ON note_enrichment_usage(note_id, user_id);
CREATE INDEX IF NOT EXISTS idx_note_tags_note_user ON note_tags(note_id);
...
CREATE INDEX IF NOT EXISTS idx_scan_data_note_id ON scan_data(note_id);
