-- First, let's see what help topics actually exist and only populate those
-- Only populate sections for topics that definitely exist

INSERT INTO public.help_topic_sections (help_topic_id, title, content, sort_order) 
SELECT ht.id, section_data.title, section_data.content, section_data.sort_order
FROM public.help_topics ht
CROSS JOIN (
  VALUES
    -- Creating Study Goals sections
    ('Getting Started with Study Goals', 
...
) AS section_data(title, content, sort_order)
WHERE ht.title = 'Creating Study Goals';

-- SMART Goal Framework sections  
INSERT INTO public.help_topic_sections (help_topic_id, title, content, sort_order) 
SELECT ht.id, section_data.title, section_data.content, section_data.sort_order
FROM public.help_topics ht
CROSS JOIN (
...
WHERE ht.title = 'SMART Goal Framework';

-- Add sections for other guaranteed topics
INSERT INTO public.help_topic_sections (help_topic_id, title, content, sort_order) 
SELECT ht.id, section_data.title, section_data.content, section_data.sort_order
FROM public.help_topics ht
CROSS JOIN (
...
WHERE ht.title = 'Creating Tasks';

-- Add Analytics Dashboard sections
INSERT INTO public.help_topic_sections (help_topic_id, title, content, sort_order) 
SELECT ht.id, section_data.title, section_data.content, section_data.sort_order
FROM public.help_topics ht
CROSS JOIN (
...
WHERE ht.title = 'Analytics Dashboard';