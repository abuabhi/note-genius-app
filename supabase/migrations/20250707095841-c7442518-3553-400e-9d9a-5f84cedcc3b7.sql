-- Fix the auto-generation issue by changing default values and cleaning up existing data

-- First, update existing notes that have 'pending' status with no actual content to NULL
UPDATE public.notes 
SET 
  summary_status = NULL,
...
WHERE 
  (summary_status = 'pending' AND (summary IS NULL OR summary = ''))
  AND (key_points_status = 'pending' AND (key_points IS NULL OR key_points = ''))
  AND (markdown_content_status = 'pending' AND (markdown_content IS NULL OR markdown_content = ''))
  AND (improved_content_status = 'pending' AND (improved_content IS NULL OR improved_content = ''))
  AND (enriched_status = 'pending' AND (enriched_content IS NULL OR enriched_content = ''));

-- Update individual fields that are 'pending' with no content
UPDATE public.notes 
